...
function ColorlibStepIcon(props) {
  ...
  return (
    <ColorlibStepIconRoot ownerState={{ completed, active }} className={className}>
      <Typography fontFamily='inter' fontWeight='600' fontSize='24px' >{String(props.icon)}</Typography>
...
export default function ProgressBar({step,steps}) {
  return (
      ...
      <Stepper alternativeLabel activeStep={(step-1)} connector={<ColorlibConnector/> }  >
        {steps.map((label) => (
          <Step key={label}>
            <StepLabel StepIconComponent={ColorlibStepIcon}><Typography fontFamily='inter' fontWeight='600' fontSize='14px' >{label}</Typography></StepLabel>
          </Step>
        ))}
      </Stepper>
